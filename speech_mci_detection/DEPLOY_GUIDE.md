...
### 3.2 从旧版（早期部署、尚未打 tag）升级到最新修复

早期服务器是在打 tag 之前直接从 `main` 拉的旧提交部署的（对应 2026-08-29 前后）；本次最新修复同样未打 tag（见「六、版本记录」`main` 行）。两端的代码都在 `main` 分支上，升级方式为切到 `main` 后快进：

```bash
cd <部署目录>
git fetch origin
git checkout main                    # 若当前停在旧 tag（detached HEAD），先切回 main 分支
git pull origin main                 # 快进到最新修复（一次带过 deploy-v20260830/31、v20260901 全部变更）
git lfs pull                         # 确保 pkl / stanza 模型等大文件真实落地
# 本次修复未新增 Python 依赖，无需 pip install
# 重启服务
```

> 说明：旧部署点先于 `deploy-v20260830`，本次 `git pull` 会一次性快进经过 `deploy-v20260830 → deploy-v20260831 → deploy-v20260901 → main 修复`，模型权重（`my_severity_combined.pkl`）与推理代码随之一起更新。更新后确认 pkl 是 500+KB 实际文件（而非 100 字节 LFS 指针）。
...
| Tag                 | 日期         | 关键变更                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 升级动作                                                                                      |
| ------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `main`（本次修复，未打 tag） | 2026-08-31 | **消除域外数据的 0.0/50.0 端点分 + 特征截断（Winsorize）**：① `low_confidence` 且完整分饱和在端点（`saturated`）的样本，分被 clip 钉在 0.0/100.0、无信息量——改为与极端域外一样走声学回退（`evidence=acoustic_only`）；回退分仍饱和端点（含 0.06/0.1153 这类"≈0 而非恰好 0"的极小值，因是两边界 risk 平均）的样本拉向中性带 42.5。② 推理期将每个特征截断到训练分布 `mean±5·std`（`clip_k=5`），极端 OOD 单特征（如 F0 下降斜率 std 超训练分布 14\~19σ）在线性加权里不再无限拉低决策值。实测 modma 全 48 人 0.0/50.0 全部消除（02020025→5.008、02010011→7.865、02020008→3.231，其余 2.3\~22.6 梯度声学分）；C1 分布内 316 人 `sufficient` 行为不变，AUC 0.8224→0.8212（仅 2 个低置信饱和样本受影响）。 | `git pull origin main && git lfs pull` + 重启服务（见「三、3.2」）                                   |
