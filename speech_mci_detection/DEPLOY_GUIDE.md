...
| Tag                 | 日期         | 关键变更                                                                                                                                                                                                                                                                                                                                                                          | 升级动作                                                                                      |
| ------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `main`（本次修复，未打 tag） | 2026-08-31 | **消除域外数据的 0.0/50.0 端点分（含"≈0"伪 0.0）**：`low_confidence` 且完整分饱和在端点（`saturated`）的样本，分被 clip 钉在 0.0/100.0、无信息量——改为与极端域外一样走声学回退（`evidence=acoustic_only`）；回退分仍饱和端点（含 0.06/0.1153 这类"≈0 而非恰好 0"的极小值，因是两边界 risk 平均）的样本拉向中性带 42.5。实测 modma 全 48 人 0.0/50.0 全部消除（02020025/02010011/02020008→42.5，其余→2.3\~22.6 梯度声学分）；C1 分布内 316 人 `sufficient` 行为不变，AUC 0.8224→0.8222（仅 2 个低置信饱和样本受影响）。 | `git pull origin main && git lfs pull` + 重启服务                                             |
